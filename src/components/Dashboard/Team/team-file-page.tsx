...
export function TeamFilePage({ user }: TeamFilePageProps) {
  ...
  const isCoach = true; // TODO: change this to user.groupsAsCoach.some((group) => group.id === groupId);
    ...
    <div className="container mx-auto py-12 px-4 md:px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {groupFiles && groupFiles.length > 0 ? (
          groupFiles.map((file: DBFileType) => (
            <TeamFileCard isCoach={isCoach} file={file} key={file.id} />
          ))
        ) : (
